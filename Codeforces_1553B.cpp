...
{
  if(a==b) return true;
  if(b>a or a%3!=0) return false;
  return  find(a-a/3,b) or find(a/3,b);
}
     ...
     if(s[i]==t[cnt])
     {
        for(int j=i;j<n;j++)
        {
            if(s[j]==t[cnt])
            {
                cnt++;
                if(j==0) continue;
                string ans=t.substr(0,cnt);
                string k=s.substr(0,j);
                reverse(all(k));
                ans+=k;
                while(ans.size()>t.size()) ans.pop_back();
                if(ans==t)
                {
                    yess;
                    return;
...
// probelm link : https://codeforces.com/contest/1553/problem/B