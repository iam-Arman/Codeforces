...
void solve()
{
  ...
  ll myhealth,myattack,monsterattack,monsterhealth;
  cin>>myhealth>>myattack>>monsterhealth>>monsterattack;
  ...
  for(int i=0;i<=k;i++)
  {
     ll currenthealth=myhealth+(i*health);

     ll currentattack=myattack+(k-i)*attack;

     ll mytimetokillmonsster=(monsterhealth+currentattack-1)/currentattack;

     ll monstertimetokillme=(currenthealth+monsterattack-1)/monsterattack;

     if(monstertimetokillme>=mytimetokillmonsster)
     {
      cout<<"YES"<<el;
      return;
     }
