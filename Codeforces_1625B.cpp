...
string pal(vector<int>& vec,int i,int j)
{
   if(i>=j)
   {
     return "YES";
...
//    }
   ...
   for(int i=0;i<n-1;i++)
   {
     ...
     {
        int k=n-vec[i+1].second+vec[i].second;  
        ans=max(ans,k); 
        // cout<<vec[i].first<<sp<<vec[i].second<<sp<<vec[i+1].first<<sp<<vec[i+1].second<<sp<<k<<el;
     }
   }
...
// problem link : https://codeforces.com/contest/1625/problem/B