...
void dfs(int node,int parent,vector<int> graph[],vector<int>& vis)
{
    vis[node]=1;
    for(auto x: graph[node])
    {
        if(vis[x]==0 ) dfs(x,node,graph,vis);
        else if(vis[x]==1 and x!=parent) ok=true;
    }
...
void solve()
{
   ...
   {
    ok=false;
     if(vis[i]==0 and graph[i].size()>0)
     {
        dfs(i,-1,graph,vis); 
        mx++; // counting connected component 
     }
     if(ok) mn++; // detecting number of cycle 
   }

   if(mn>mx) mn=mx; // if all the connected component are cycle then min and max round will be same 
