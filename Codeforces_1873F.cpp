...
bool chk(vector<int> &fruit,vector<int> & height,int k,int mid)
{
    int n=siz(fruit);
    int cnt=1;
    ll total=fruit[0];
    ...
    for(int i=1;i<n;i++)
    {
        ...
        if(height[i-1]%height[i]==0 and total+fruit[i]<=k)
        {
            cnt++;
            total+=fruit[i];
            has.pb(fruit[i]);
        }
        else if(height[i-1]%height[i]==0 and total+fruit[i]>k)
        {
           while(total+fruit[i]>k and has.size())
           {
               total-=has.front();
               has.pop_front();
               cnt--;
...
void solve()
{
    ...
    for(auto & it: fruit) cin>>it;
    for(auto & it: height) cin>>it;


    int MinFruit=*min_element(all(fruit));
    if(MinFruit>k) 
    {
        cout<<0<<el;
        continue;
    ...
    while(high-low>1)
    {
        ...
        bool ok=chk(fruit,height,k,mid);
