...
void solve()
{
  ...
  int initial_time=((n+k-1)/k)*t;

  int cake_cooked_while_build=(d/t)*k;

  n-=cake_cooked_while_build;

  if((n+1)/2<2*k) n=2*k;

  int time_after_oven=((n+k-1)/k)/2;
  time_after_oven=(time_after_oven*t)+d;

  if(time_after_oven<initial_time)
  {
    cout<<"YES"<<endl;
  }
