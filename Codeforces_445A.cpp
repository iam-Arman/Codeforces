...
void dfs(int i,int j)
{
  ...
  if(vis[i][j]==1 or graph[i][j]!='.') return ;
  
    vis[i][j]=1;
   
        if(graph[i+1][j]!='B' and graph[i-1][j]!='B' and graph[i][j+1]!='B' and graph[i][j-1]!='B')
        {
            graph[i][j]='B';
        }
        else graph[i][j]='W';
    ...
    for(int k=0;k<4;k++)
    {
      ...
      if(vis[x][y]==0 and (x>=1 and x<=n and y>=1 and y<=m ) ) dfs(x,y);
