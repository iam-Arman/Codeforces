...
ll bigmod(ll base,ll power){  ll mod=998244353; ll res=1; ll p=base%mod; while(power>0) { if(power%2==1) {  res=((res%mod)*(p%mod))%mod; }  power/=2; p=((p%mod)*(p%mod))%mod; } return res; }
...
void solve()
{
  ...
  while(low<=high)
  {
     ...
     if(ans*(ans+1)/2<k) ans++;

     ans=min(ans,n);


     ll rem=k-1LL*ans*(ans+1)/2;
