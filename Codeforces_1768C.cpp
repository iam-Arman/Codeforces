...
void solve()
{
   ...
   for(int i=0;i<n;i++)
   {
    if(v1[i]==0)
    {
      int k=st1.order_of_key(v2[i]);
      k=*st1.find_by_order(k-1);
      v1[i]=k;
      st1.erase(k);
    }
   else if(v2[i]==0)
    {
      int k=st2.order_of_key(v1[i]);
      k=*st2.find_by_order(k-1);
      v2[i]=k;
      st2.erase(k);
    }
   }
   ...
   for(int i=0;i<n;i++)
   {
    m1[v1[i]]++;
    m2[v2[i]]++;
    if((v1[i]==0 or v2[i]==0) or max(v1[i],v2[i])!=vec[i] )
    {
      cout<<"NO"<<el;
      return;
    }
    if(m1[v1[i]]>1 or m2[v2[i]]>1)
    {
      cout<<"NO"<<el;
      return;
    }
